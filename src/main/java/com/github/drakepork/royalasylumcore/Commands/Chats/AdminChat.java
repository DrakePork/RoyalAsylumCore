	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(sender instanceof Player) {
			...
			if(args.length > 0) {
				...
				String format = langConf.getString("chat.admin.format").replaceAll("\\[name\\]", Matcher.quoteReplacement(player.getName()));
				String message = format.replaceAll("\\[message\\]", Matcher.quoteReplacement(cMessage));
				for (Player online : Bukkit.getServer().getOnlinePlayers()) {
					if (online.hasPermission("royalasylum.chat.admin")) {
				...
				if(plugin.stickyChatEnabled.containsKey(player.getUniqueId())) {
					if (plugin.stickyChatEnabled.get(player.getUniqueId()).equals("admin-chat")) {
						String stickEnabled = langConf.getString("chat.stickied.disabled").replaceAll("\\[chat\\]", "Admin");
						...
						stickSwapped = stickSwapped.replaceAll("\\[newchat\\]", "Admin");
						player.sendMessage(colourMessage(prefix + stickSwapped));
						plugin.stickyChatEnabled.put(player.getUniqueId(), "admin-chat");
					}
				} else {
					String stickEnabled = langConf.getString("chat.stickied.enabled").replaceAll("\\[chat\\]", "Admin");
					player.sendMessage(colourMessage(prefix + stickEnabled));
					plugin.stickyChatEnabled.put(player.getUniqueId(), "admin-chat");
				}
			}
			...
			if(args.length > 0) {
				...
				String format = langConf.getString("chat.admin.format").replaceAll("\\[name\\]", "Console");
				String message = format.replaceAll("\\[message\\]", Matcher.quoteReplacement(cMessage));
				for (Player online : Bukkit.getServer().getOnlinePlayers()) {
					if (online.hasPermission("royalasylum.chat.admin")) {
						online.sendMessage(colourMessage(message));
				...
				tellConsole(colourMessage(prefix + langConf.getString("chat.admin.wrong-usage")));
			}
		}
		return true;
	}
